...
#resource : https://github.com/udaysagar2177/DES
#resource : https://medium.com/@ziaullahrajpoot/data-encryption-standard-des-dc8610aafdb3
...
def precompute():
    ...
    # calculation of 8-bit list for every character
    for n in range(128):
        b = [0,0,0,0,0,0,0,0]
        for i in range(0, 8):
            if(n%2):
    ...
    # calculation of char from string of 8-bits
    k = 0
    binary_to_text = {}
    for i in to_binary:
        string = ''
...
#exapansion permutation untuk expan 32 bit block menjadi 48 bit block
...
#s-box function, reduce 48 bit block menjadi 32 bit block
...
#iterating code untuk 16 xor_rounds
...
#pwnambahan bits untuk mengubah integer number of 64-bit blocks
def apply_pads():
    global text_bits
...
def main():
    ...
    if(choice == 1):
        ...
        for i in range(0, len(text_bits), 64):
            final_cipher += encryption_DES(i, (i+64))

        # mengubah binary cipher menjadi hex-decimal
        hex_cipher = ''
        i = 0
        ...
        print('Chiper Hasil Enkripsi')
        print(hex_cipher)
        
    else:
        print('Masukkan Chiper Teks')
        cipher = str(input())
        text_bits = []
        ciphertext = ''
         #mwngubah hex-decimal menjadi binary form
        for i in cipher:
        ...
        bin_mess = ''
        for i in range(0, len(text_bits), 64):
            bin_mess += decryption_DES(i, (i+64))
